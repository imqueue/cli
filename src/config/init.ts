...
export async function selectTemplate(type: string): Promise<{
    ...
    if (type === 'repo') {
        ...
        if (!(answer.tplDir && answer.tplDir.trim())) {
            throw new TypeError(
                'Template directory path expected, ' + 'but was not given!',
            );
        }
    ...
    let answer: any = await inquirer.prompt<{ useDefault: boolean }>([
        {
            ...
            name: 'useDefault',
            message:
                'Do you want to use default template for newly created ' +
                'services?',
            default: true,
        },
    ]);
    ...
    answer = await inquirer.prompt<{ saveGitHubToken: boolean }>([
        {
            ...
            message:
                'Would you like to save GitHub auth token in a local config ' +
    ...
    answer = await inquirer.prompt<{ isPrivate: boolean }>([
        {
            ...
            name: 'isPrivate',
            message:
                'Does created service should a private repository on GitHub?',
            default: true,
        },
    ]);
    ...
    answer = await inquirer.prompt<{ saveDockerCredentials: boolean }>([
        {
            ...
            message:
                'Would you like to store docker credentials locally to ' +
